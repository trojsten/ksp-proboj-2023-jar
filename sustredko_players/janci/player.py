...
class MyPlayer(ProbojPlayer):
    ...
    def make_turn(self) -> Turn:
        ...
        if nearestEntity:
            ...
            # don't ask why, i have no fuckin idea
            if self.isInRange(nearestEntity, stats):
                pos = self.myself.position
        ...
        for player in self.players.values():
            if player != self.myself:
                ...
                if not self.isOutOfBounds(player) and self.isInRange(player, stats) and self.myself.position.distance(player.position) < (self.myself.position.distance(nearestPlayer.position) if nearestPlayer != None else float('inf')):
