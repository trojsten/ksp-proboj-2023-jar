...
class MojPlayer(ProbojPlayer):
    def make_turn(self) -> Turn:
        ...
        if self.world.max_x * 2 < 500:
            if (go_pos.y < self.world.min_y):
                go_pos.y = self.world.min_y
            if (go_pos.y > self.world.max_y):
                go_pos.y = self.world.max_y
            if (go_pos.x < self.world.min_x):
                go_pos.x = self.world.min_x
            if (go_pos.x > self.world.max_x):
                go_pos.x = self.world.max_x
        else:  
