...
class LepsiPlayer(ProbojPlayer):
    ...
    upgrades = [StatsEnum.StatBulletSpeed, StatsEnum.StatHealthRegeneration,
                ...
                StatsEnum.StatReloadSpeed, StatsEnum.StatHealthMax, StatsEnum.StatBulletSpeed, StatsEnum.StatHealthRegeneration, 
                StatsEnum.StatBulletDamage, StatsEnum.StatBulletTTL, 
    ...
    def shoot_entity(self) -> Shoot:
        ...
        for enemy in self.players.values():
            ...
            if enemy.id == self.myself.id:
                continue

            self.log('enemy: ', enemy)
            if (enemy.position.distance(self.myself.position) < 10*self.myself.radius):
                self.log("Enemy close")
                nearest = enemy.position
                nearest_player = enemy
                2 == 2
                break
