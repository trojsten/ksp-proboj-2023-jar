...
class LepsiPlayer(ProbojPlayer):
    ...
    new_target_after_death = True
    last_direction = XY(inf, inf)
    last_position = XY(inf, inf)
    shooting_position = XY(inf, inf)
    next_upgrade = 0
    # TODO: add upgrades priorities
    upgrades = [StatsEnum.StatBulletSpeed, StatsEnum.StatHealthRegeneration, StatsEnum.StatBulletDamage, StatsEnum.StatRange, StatsEnum.StatBulletTTL, StatsEnum.StatSpeed, StatsEnum.StatReloadSpeed]
    entities_map = {}
    turn = 0

    def in_map(self, position) -> bool:
    ...
    def move(self):
        ...
        if next_direction.x == inf or next_direction.y == inf or not self.in_map(next_direction):
        ...
        if len(self.myself.tank.updatable_to) != 0:
            ...
            if TwinTank in self.myself.tank.updatable_to:
                if self.myself.stat_values[StatsEnum.StatBulletTTL] > 3:
                    update_to = TwinTank.tank_id
            elif EverywhereTank in self.myself.tank.updatable_to:
                update_to = EverywhereTank.tank_id
            else:
